...
export async function resolvePostProps({
  ...
  const normalizePathLike = (value, { stripHtml = false } = {}) => {
    if (!value || typeof value !== 'string') return ''
    let v = decodeURIComponent(value).trim()
    if (v.startsWith('/')) v = v.substring(1)
    if (v.endsWith('/')) v = v.substring(0, v.length - 1)
    if (stripHtml && v.endsWith('.html')) {
      v = v.substring(0, v.length - 5)
    }
    return v
  }

  // Support Notion share-like paths such as:
  // /OpenClaw-Dashboard-Agent-30b91a74161c8099b4d7c0dfec8ee4e8
  const extractTrailingPageId = value => {
    const normalized = normalizePathLike(value, { stripHtml: true })
  ...
  const fullSlugRaw = normalizePathLike(segments.join('/'))
  const fullSlug = normalizePathLike(fullSlugRaw, { stripHtml: true })
  const lastSegmentRaw = segments[segments.length - 1]
  const lastSegment = normalizePathLike(lastSegmentRaw, { stripHtml: true })
  const extractedPageId = extractTrailingPageId(lastSegment || fullSlug)
  ...
  if (fullSlug) {
    post = props?.allPages?.find(p => {
      ...
      const postSlug = normalizePathLike(p?.slug, { stripHtml: true })
      const postHref = normalizePathLike(p?.href, { stripHtml: true })
      return postSlug === fullSlug || postHref === fullSlug
    })
  }
  ...
  if (!post && fullSlug) {
    post = props?.allPages?.find(p => {
      const postId = normalizePathLike(p?.id)
      return postId === fullSlug
    })
  }

  /**
   * 4.5️⃣ 兼容 Notion 分享链接：末尾含 page id 时按 id 匹配
   */
  if (!post && extractedPageId) {
    post = props?.allPages?.find(p => {
      const postId = normalizePathLike(p?.id)
      return postId === normalizePathLike(extractedPageId)
    })
  }
  ...
  if (
    !post &&
    typeof (extractedPageId || lastSegment) === 'string' &&
    (extractedPageId || /^[a-f0-9-]{32,36}$/i.test(lastSegment))
  ) {
    try {
      post = await fetchPageFromNotion(extractedPageId || lastSegment)
    } catch (e) {
      console.warn(
        '[resolvePostProps] fetchPageFromNotion failed:',
        extractedPageId || lastSegment,
        e
      )
    }
  }
...
function cleanBlock(item) {
  ...
  if (pageBlock) {
    for (const i in pageBlock) {
      ...
      delete pageBlock[i]?.value?.crdt_data
      delete pageBlock[i]?.value?.crdt_format_version
    }
  }
  return post
}
